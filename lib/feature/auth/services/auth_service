...
class AuthService {
  ...
  Future<UserModel?> signInWithGoogle() async {
    try {
      ...
      try {
        ...
        LoggerService.warning('Warning: Could not clear previous session', error: e);
      }
      ...
      if (googleUser == null) {
        try {
          ...
          LoggerService.error('Google Sign-In account selection failed', error: e);
        }
      ...
      if (user == null) {
        LoggerService.error('Firebase user is null after successful authentication');
        await _googleSignIn.signOut();
      ...
      try {
        ...
        LoggerService.info('User model fetched from Firestore: ${userModel?.email}');
      } catch (e) {
      ...
      if (userModel == null) {
        try {
          LoggerService.info('Creating new user with role: ${UserRole.recipient.name}, type: ${UserType.user.name}');
          ...
          LoggerService.info('New user created: ${user.email} with role: ${UserRole.recipient.name}');
          ...
          LoggerService.error('Failed to create user in Firestore', error: createError);
          
          try {
            await _firebaseAuth.signOut();
            await _googleSignIn.signOut();
          } catch (signOutError) {
            LoggerService.error('Error during cleanup after creation failure', error: signOutError);
          }
        ...
        if (userModel.status == UserStatus.pending) {
          await _firebaseAuth.signOut();
          await _googleSignIn.signOut();
          throw Exception('Your account is pending approval. Please wait for confirmation.');
        }
      ...
      LoggerService.info('Google sign-in completed successfully for user: ${user.email}');
      return userModel;
    } on FirebaseAuthException catch (e) {
      LoggerService.error('Firebase Auth error during Google sign-in', error: e);
      
      try {
        await _firebaseAuth.signOut();
        await _googleSignIn.signOut();
      } catch (signOutError) {
        LoggerService.warning('Error during cleanup sign out', error: signOutError);
      }
  ...
  UserRole _determineUserRoleFromEmail(String email) {
    ...
    if (emailPrefix.contains('admin') || 
        ...
        emailPrefix.startsWith('dvc')) { // Deputy Vice Chancellor
    ...
    if (emailPrefix.contains('staff') || 
        ...
        emailPrefix.length < 6) { // Usually staff have shorter IDs
      return UserRole.certificateAuthority;
    }
  ...
  Future<UserModel?> signInWithEmailAndPassword(String email, String password) async {
    try {
      ...
      if (userModel == null) {
        ...
        LoggerService.info('User ${user.uid} found in Auth but not Firestore. Creating now...');
      ...
      if (userModel.status == UserStatus.suspended) {
        throw Exception('Your account has been suspended. Contact administrator.');
  ...
  Future<UserModel?> signUpWithEmailAndPassword(
    ...
    try {
      ...
      if (!AppConfig.isValidUpmEmail(email)) {
        throw Exception('Registration requires a valid UPM email address (@upm.edu.my)');
      }

      // Additional validation for admin/CA roles
      if ((userType == UserType.ca || userType == UserType.admin)) {
        final emailPrefix = email.toLowerCase().split('@').first;
        if (userType == UserType.admin && !_isValidAdminEmail(emailPrefix)) {
          throw Exception('System administrator role requires valid admin credentials');
        }
      }
      ...
      final userStatus = (userType == UserType.admin || userType == UserType.ca) 
          ? UserStatus.pending 
          : UserStatus.active;
      ...
      final userModel = await _createUserFromFirebaseUser(user, role, displayName, userStatus, userType);
      ...
      LoggerService.info('Account created successfully for user: $email with role: ${role.name}');
      return userModel;
    } on FirebaseAuthException catch (e) {
  ...
  Future<void> signOut() async {
    try {
      ...
      LoggerService.info('User signed out successfully from Firebase and Google');
    } catch (e) {
  ...
  Future<UserModel> _createUserFromFirebaseUser(
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (verifyUser != null) {
          LoggerService.info('User creation verified successfully: ${firebaseUser.uid}');
          break;
        }
      } catch (e) {
        LoggerService.warning('User verification attempt ${retryCount + 1} failed: $e');
      }
      ...
      if (retryCount < maxRetries - 1) {
        ...
        LoggerService.error('Failed to verify user creation after $maxRetries attempts');
        throw Exception('User creation verification failed - please try again');
      }
    }
    
    if (verifyUser == null) {
      LoggerService.error('User verification failed - user not found in Firestore');
      throw Exception('User creation verification failed - please try again');
    }
  ...
  String _extractDepartmentFromEmail(String email) {
    ...
    if (emailPrefix.startsWith('fst')) return 'Faculty of Science and Technology';
    if (emailPrefix.startsWith('fpe')) return 'Faculty of Engineering';
    if (emailPrefix.startsWith('fpm')) return 'Faculty of Medicine and Health Sciences';
    if (emailPrefix.startsWith('fpv')) return 'Faculty of Veterinary Medicine';
    if (emailPrefix.startsWith('fpe')) return 'Faculty of Economics and Management';
    if (emailPrefix.startsWith('fbmk')) return 'Faculty of Modern Languages and Communication';
    if (emailPrefix.startsWith('fs')) return 'Faculty of Science';
    if (emailPrefix.startsWith('fp')) return 'Faculty of Agriculture';
    if (emailPrefix.startsWith('frsb')) return 'Faculty of Forestry and Environment';
  ...
  Exception _handleFirebaseAuthException(FirebaseAuthException e) {
    switch (e.code) {
      ...
      case 'weak-password':
        return Exception('Password must be at least ${AppConfig.passwordMinLength} characters');
      case 'invalid-email':
